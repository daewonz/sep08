...
<title>board</title>
	...
	$(function(){
		...
		$(".detail").click(function(){
			let bno = $(this).children("td").eq(0).children(".hiddenBno").val();
			let rowNum = $(this).children("td").eq(0).html();
	...
	$.ajax({
		...
		success:function(data){
			...
			$(".detail-name").html(data.m_name);
			$(".detail-date").text(data.bdate);
			$(".detail-read").text(data.bip + " 조회수 " + data.blike);
        ...
        <header class="masthead">
            <div class="container">
               ...
               <table class="table table-dark table-hover table-striped">
               		...
               		<tr class="row" onclick="detail(${row.abno})">
               			<td class="col-1">${row.abno}</td>
               			<td class="col-5 title">${row.abtitle}<c:if test="${row.commentcount ne 0 }">&nbsp;<span class="badge bg-secondary">${row.commentcount}</span></c:if></td>
               			<td class="col-2">${row.abwrite}</td>
               			<td class="col-2">${row.abdate}</td>
               			<td class="col-1">${row.ablike}</td>
               ...
               <button type="button" class="btn btn-secondary" onclick="location.href='./annonywrite'">글쓰기</button>
            </div>
        </header>
